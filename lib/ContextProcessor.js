...
export class ContextProcessor {
   ...
   */
  async applyEmbeddedContexts({activeCtx, obj}) {
    // push any local embedded contexts onto the context stack
    const localContexts = obj['@context'];
    return this._updateActiveContext({activeCtx, contexts: localContexts});
  }

  async applyTypeScopedContexts({activeCtx, objectTypes}) {
  ...
  async _updateActiveContext({activeCtx, contexts}) {
    // no new contexts
    if(!contexts) {
      return activeCtx;
    }
    ...
    for(const context of contexts) {
      const entry = await this._loadContext({context});
