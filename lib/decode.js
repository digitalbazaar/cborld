...
export async function decode({
  cborldBytes, documentLoader, compressionMap = new Map(), diagnose}) {
  ...
  if(cborldBytes[index++] !== 0x06) {
    throw new CborldError(
      'ERR_NOT_CBORLD', 'CBOR-LD 16-bit tag must start with `0x06`.');
  }
  ...
  // decompress CBOR-LD
  const decompressor = new Decompressor({documentLoader, compressionMap});
