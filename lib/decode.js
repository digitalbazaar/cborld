...
export async function decode({
  ...
  // process payload with new tag
  if(cborldBytes[1] == CBORLD_TAG_SECOND_BYTE) {
    // get suffix to decompress
    const suffix = _getSuffixFromBytesWithVarint({cborldBytes});
  ...
  if(cborldBytes[1] == CBORLD_TAG_SECOND_BYTE_LEGACY) {
    // legacy support - process CBOR with old tag
    let index = 2;
...
function _getSuffixFromBytesWithVarint({cborldBytes}) {
  let index = 2;
  let suffix;
