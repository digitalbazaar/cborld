...
* @param {allowEmbedded} [options.allowEmbedded=false] - Whether or not
*   embedded, non-remote contexts are permitted.
*
* @returns {Array} - A list of JSON-LD Context URLs.
*/
export function getJsonldContextUrls({jsonldDocument, allowEmbedded = false}) {
  const contextUrls = [];
  ...
  for(const [key, value] of Object.entries(jsonldDocument)) {
    if(key === '@context' || key === '@import') {
      ...
      for(const context of contexts) {
        ...
        if(typeof context !== 'string') {
          ...
          contextUrls.push(...getJsonldContextUrls(
            {jsonldDocument: context, allowEmbedded}));
        } else {
      ...
      contextUrls.push(...getJsonldContextUrls(
        {jsonldDocument: value, allowEmbedded}));
    } else if(Array.isArray(value)) {
      value.forEach(element => {
        if(typeof element === 'object') {
          contextUrls.push(...getJsonldContextUrls(
            {jsonldDocument: element, allowEmbedded}));
        }
      });
    }
  }
