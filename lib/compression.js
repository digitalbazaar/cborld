...
export async function toCborld({
  ...
  const encodingMap = _generateEncodingMap(jsonldDocument, compressionMap);

  console.log("ENCODING MAP");
  console.dir(encodingMap, {depth: 1});
...
function _generateCompressionMap(context) {
  ...
  for(const [key, value] of Object.entries(context['@context'])) {
    ...
    if(typeof value === 'string') {
      ...
      if(value['@context']) {
        const scopedCompressionMap = _generateCompressionMap(value);
        compressionMap = {...compressionMap, ...scopedCompressionMap};
      }
    } else {
...
function _generateEncodingMap(jsonldDocument, compressionMap) {
  ...
  for(const [key, value] of Object.entries(jsonldDocument)) {
    ...
    if(typeof value === 'object' && value.constructor === Object) {
      encodedValue = _generateEncodingMap(value, compressionMap);
    } else if(compressionMap) {
