...
export async function toCborld({
  ...
  for(const [key, value] of Object.entries(jsonldDocument)) {
    cborMap.set(compressionDictionary[key].compressedTerm, value);
  }

  const cborldBytes = cbor.encode(cborMap);

  if(options.debug) {
    console.log(await cbor.Commented.comment(cborldBytes, {max_depth: 20}));
    console.log('CBOR-LD buffer size:', cborldBytes.length);
  }
