...
export async function encode({
  ...
  if(appContextMap){
    ...
    const stringTableWithAppContext = new Map(KEYWORDS_TABLE);

    for(let i=0; i<appContextMap.length; i++){
      stringTableWithAppContext.set(appContextMap[i][0], appContextMap[i][1]);
    }
    if(compressionMode === 0) {
      ...
      // compress CBOR-LD
      const compressor = new Compressor({documentLoader, appContextMap: stringTableWithAppContext});
