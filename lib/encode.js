...
async function _generateEncodingMap2({
  ...
  const contextMap = await _getContexts({jsonldDocument, documentLoader});
...
async function _getContexts({jsonldDocument, documentLoader}) {
  ...
  for(const url of contextUrls) {
    promises.push(_getDocument({url, documentLoader}));
  }
  ...
  for(let i = 0; i < contextUrls.length; ++i) {
    const url = contextUrls[i];
    const {'@context': context} = docs[i];
    if(!context) {
      // FIXME: handle contexts with arrays and that import other contexts...
      // the `getJsonldContextUrls` function presently doesn't cover these
      // cases nor does it handle `@import` nor does it have a request queue
