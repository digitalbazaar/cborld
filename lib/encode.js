 ...
 * @param {number} [options.registryEntryId=1] - The registry
 *   entry ID for the registry entry associated with the resulting CBOR-LD
 *   payload. For legacy support, use 'legacyFormat' = 'range'.
 ...
 * @param {Map} [options.appContextMap] - For use with the 'singleton' value of
 *   `legacyFormat`.
 * @param {number} [options.compressionMode] - For use with the 'singleton' value of
 *   `legacyFormat`.
 * @param {string} [options.legacyFormat] - Specify whether to use the singleton
 *   legacy tag system ('singleton') or the range legacy tag system ('range').
 *
...
export async function encode({
  ...
  if(!(typeof registryEntryId === 'number' && registryEntryId >= 0)) {
    throw new TypeError(
      '"registryEntryId" must be a non-negative integer.');
  }

  // validate that an acceptable value for `legacyFormat` was passed
  if((legacyFormat !== undefined) && !(legacyFormat === 'singleton' || legacyFormat === 'range')) {
    throw new TypeError(
      '"legacyFormat" must be "singleton" (tags 1280/1281) or "range" ' +
      '(tags 1536-1791).');
  }
  ...
  const isLegacySingleton = legacyFormat === 'singleton';
  const isLegacyRange = legacyFormat === 'range'
  if(isLegacySingleton) {
    ...
    // use default values in legacy singleton mode
    if(compressionMode === undefined) {
      compressionMode = 1;
    }
  }
  if(!isLegacySingleton) {
    if(compressionMode !== undefined) {
      throw new TypeError(
        '"compressionMode" must only be passed in legacy singleton mode');
    }
  }
  ...
  if(compressionMode === 0 || registryEntryId === 0) {
    ...
    if(legacyFormat !== 'singleton') {
      ...
      if(!typeTable && registryEntryId === 1) {
  ...
  const prefix = _getPrefix({
    isLegacySingleton, isLegacyRange, compressionMode, registryEntryId
  });
...
function _getPrefix({
  ...
  return new Uint8Array([
    0xd9, // CBOR major type 6 + 2 byte tag size
    0xcb, 0x1d, // non-legacy CBOR-LD tag
    ...[
      0x82, // 2 element array
