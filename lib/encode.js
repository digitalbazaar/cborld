...
export async function toCborld(
  ...
  // TODO: use new Compressor/Decompressor
  {
    ...
    // FIXME: should return cborldBytes instead
    //const cborLdBytes = await compressor.compress({jsonldDocument, diagnose});
    ...
    // FIXME: might want to do cborld decode first and pass the result to
    // compressor ... because we don't know if its compressed or not before
    // handing it to the decompressor otherwise
    await decompressor.decompress({cborldBytes: suffix});
  }
