...
async function _recursiveGenerateEncodingMap({
  ...
  if(context) {
    encodingMap.set(
      CONTEXT_TERM_ID, new ContextEncoder({context, appContextMap}));
  }
  ...
  for(const key of keys) {
    ...
    const termId = _getTermId({term: key, termToId});
...
async function _decode({encodedMap}) {
...
async function _recursiveDecode({
  ...
  if(encodedContext) {
    const decoder = new ContextDecoder();
    obj['@context'] = decoder.decode({encoded: encodedContext});
  }
  ...
  for(const [key, encoded] of encodedMap) {
    if(key === CONTEXT_TERM_ID) {
      // context already processed first
      continue;
    }

    // FIXME: determine if `key` will be checked as a string/number to support
    // uncompressed keys

    // check for undefined term IDs
    const term = idToTerm(key);
    if(term === undefined) {
      throw new CborldError(
    ...
    if(term === '@id' || aliases.id.has(term)) {
      ...
      if(decoder) {
        obj[term] = decoder.decode({encoded});
      } else {
        obj[term] = encoded;
      }
      continue;
    ...
    // there are multiple values if encoded is a two dimensional array
    const isArray = Array.isArray(encoded) && Array.isArray(encoded[0]);
    const encodedValues = isArray ? encoded : [encoded];
    const entries = [];
...
function _getTermId({term, termToId}) {
...
async function _getActiveContext({
  ...
  await _updateContextStack({
    contextStack, contextMap, termToId, idToTerm,
  ...
  for(const type of totalTypes) {
    ...
    if(contexts) {
      await _updateContextStack({
        contextStack, contextMap, termToId, idToTerm, contexts,
  ...
  for(const context of contexts) {
    ...
    if(!entry) {
      ...
      entry = await _addContext({
        context: ctx, contextUrl, contextMap, termToId, idToTerm,
        documentLoader
      });
    }
...
async function _addContext({
  context, contextUrl, contextMap, termToId, idToTerm, documentLoader
}) {
  // handle `@import`
  ...
  if(importUrl) {
    ...
    if(!importEntry) {
      ...
      importEntry = await _addContext({
        context: importCtx, contextUrl: importUrl, contextMap,
        termToId, idToTerm, documentLoader
      });
    }
