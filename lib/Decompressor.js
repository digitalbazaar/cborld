...
import {UrlDecoder} from './codecs/UrlDecoder.js';
  ...
  constructor({
    ...
    if(typedLiteralTable) {
      for(const [k, map] of typedLiteralTable) {
        this.reverseTypedLiteralTable.set(k, reverseMap(map));
      }
    }
  }
      ...
      if(value !== undefined) {
        if(Array.isArray(value)) {
          obj[term] = value.map(value => {
            const decoder = UrlDecoder.createDecoder(
              {value, transformer: this});
            return decoder?.decode({value}) ?? value;
          });
        } else {
          const decoder = UrlDecoder.createDecoder({value, transformer: this});
          obj[term] = decoder?.decode({value}) ?? value;
        }
      }
    }
  }
