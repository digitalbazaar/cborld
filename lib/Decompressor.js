...
export class Decompressor extends Transformer {
  ...
  async decompress({compressedBytes} = {}) {
    ...
    console.log('decoded result', transformMap);
  ...
  _beforeTypeScopedContexts({activeCtx, obj, transformMap}) {
    ...
    for(const term of typeTerms) {
      const value = transformMap.get(termToId.get(term));
      if(value !== undefined) {
        if(Array.isArray(value)) {
          obj[term] = value.map(value => {
            const decoder = VocabTermDecoder.create({value, transformer: this});
  ...
  _getEntries({transformMap}) {
    ...
    return [...transformMap.entries()].filter(
      ([k]) => k !== CONTEXT_TERM_ID && k !== CONTEXT_TERM_ID_PLURAL);
  }
  ...
  _handleTypedValue({entries, termType, value}) {
