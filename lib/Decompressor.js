...
import {FIRST_CUSTOM_TERM_ID, STANDARDS_TABLE} from './keywords.js';
...
const CONTEXT_TERM_ID = STANDARDS_TABLE.get('@context');
...
export class Decompressor extends Transformer {
   ...
   *   (0x7FFF)).
   */
  constructor({documentLoader, compressionMap} = {}) {
    super({documentLoader, compressionMap});
    this.reverseCompressionMap = new Map();
    // build reverse contxt map
    if(compressionMap) {
      for(const [k, v] of compressionMap) {
        this.reverseCompressionMap.set(v, k);
      }
    }
  }
  ...
  async decompress({compressedBytes, diagnose} = {}) {
    ...
    this.stringToId = this.compressionMap;
    this.nextTermId = FIRST_CUSTOM_TERM_ID;
    this.idToString = new Map();
    for(const [term, id] of this.stringToId) {
      this.idToString.set(id, term);
    }
  ...
  _getEntries({transformMap, termMap}) {
    ...
    for(const [key, value] of transformMap) {
      ...
      if(def === undefined && !(term.startsWith('@') && STANDARDS_TABLE.has(term))) {
        throw new CborldError(
