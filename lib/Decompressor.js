...
export class Decompressor {
  ...
  _getEntries({activeCtx, transformMap}) {
  ...
  _getObjectTypes({activeCtx, transformMap}) {
    const objectTypes = new Set();
    // must decode object types to get their original values
    const {transformer} = this;
    const {termToId} = transformer.contextLoader;
    ...
    for(const transformMap of value) {
      if(Array.isArray(transformMap)) {
        ...
        await this._transformArray({
          activeCtx, entries: children, value: transformMap
        });
        continue;
      }
      // handle non-object
      ...
      const obj = {};
      children.push(obj);
      await this.transformer._transform({activeCtx, obj, transformMap});
    }
  ...
  async _transformObject({activeCtx, entries, value}) {
    ...
    return this.transformer._transform({
      activeCtx, obj: child, transformMap: value
    });
  }
