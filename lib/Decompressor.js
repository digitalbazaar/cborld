...
export class Decompressor {
  ...
  async decompress({transformMap} = {}) {
    ...
    const inputs = isArray ? transformMap : [transformMap];
    for(const input of inputs) {
      // FIXME: only diff w/compress() is creation of a new output
      const output = {};
      await this.transformer._transform({input, output});
      outputs.push(output);
    }
  ...
  async _transformArray({activeCtx, entries, value}) {
    ...
    for(const transformMap of value) {
      ...
      await this.transformer._transform({
        activeCtx, input: transformMap, output: obj
      });
    }
  ...
  _assignEntries({entries, termInfo, output: obj}) {
    obj[termInfo.term] = entries;
  }
}
