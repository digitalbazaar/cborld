...
export class Decompressor extends Transformer {
  ...
  constructor({
    ...
    if(urlTable) {
      for(const [k, v] of urlTable) {
        this.reverseUrlTable.set(v, k);
      }
    }
  ...
  _transformTypedValue({entries, termType, value}) {
    let decoder;
    if(this.reverseStringTable.has(value)){
      decoder = VocabTermDecoder.createDecoder(
        {value, transformer: this}
      )
    }
    else if(this.reverseTypedLiteralTable.has(termType)){
