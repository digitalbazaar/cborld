...
export class Decompressor extends Transformer {
  ...
  async decompress({cborldBytes} = {}) {
    ...
    await this._transform({obj: jsonldDocument, transformMap: encodedMap});
  ...
  _handleObjectId({obj, termInfo, value}) {
    // FIXME: rename `encoded` to `value`
    const encoded = value;
    const decoder = UriDecoder.createDecoder({encoded});
    obj[termInfo.term] = decoder ? decoder.decode({encoded}) : encoded;
  }
  ...
  _handleObjectType({obj, termInfo, value}) {
    ...
    for(const encoded of values) {
      const decoder = VocabTermDecoder.createDecoder({encoded, def, idToTerm});
  ...
  _handleTypedValue({entries, termType, value, termInfo}) {
    ...
    const decoder = DecoderClass && DecoderClass.createDecoder(
      {encoded: value, def, idToTerm});
    console.log('decoder', decoder);
    if(decoder) {
      entries.push(decoder.decode({encoded: value, idToTerm}));
      return true;
    }
  }
