...
import {CryptosuiteDecoder} from './codecs/CryptosuiteDecoder.js';
...
export const TYPE_DECODERS = new Map([
  ...
  ['https://w3id.org/security#cryptosuiteString', CryptosuiteDecoder],
...
export class Decompressor extends Transformer {
  ...
  constructor({
    ...
    this.reverseKeywordsTable = new Map();
    this.reverseStringTable = new Map();
    this.reverseUrlSchemeTable = new Map();
    this.reverseTypedLiteralTable = new Map();
    // build reverse compression tables
    if(keywordsTable) {
      for(const [k, v] of keywordsTable) {
        this.reverseKeywordsTable.set(v, k);
      }
    }
    if(stringTable) {
      for(const [k, v] of stringTable) {
        this.reverseStringTable.set(v, k);
      }
    }
    if(urlSchemeTable) {
      for(const [k, v] of urlSchemeTable) {
        this.reverseUrlSchemeTable.set(v, k);
      }
    }
    //typed literal table is map of maps, process inner map not outer
    if(typedLiteralTable) {
      for(const [k, v] of typedLiteralTable) {
        let tempReverseMap = new Map();
        for(const [k2, v2] of v){
          tempReverseMap.set(v2, k2);
        }
        this.reverseTypedLiteralTable.set(k, tempReverseMap);
      }
    }
  }
  ...
  async decompress({compressedBytes, diagnose} = {}) {
    ...
    // TODO: fix
    this.nextTermId = 100;
    this.reverseTableFromContexts = new Map();
  ...
  _beforeTypeScopedContexts({activeCtx, obj, transformMap}) {
    ...
    for(const term of typeTerms) {
      ...
      if(this.keywordsTable.has(term)) {
        termId = this.keywordsTable.get(term);
      }
      else {
        termId = tableFromContexts.get(term);
      }
  ...
  _getTermInfo({termMap, key}) {
    ...
    if(def === undefined && !(term.startsWith('@') && KEYWORDS_TABLE.has(term))) {
      throw new CborldError(
        'ERR_UNKNOWN_CBORLD_TERM',
  ...
  _transformTypedValue({entries, termType, value}) {
    let decoder;
    if(this.reverseTypedLiteralTable.has(termType)){
      decoder = TypedLiteralDecoder.createDecoder(
        {value, reverseCompressionMap: this.reverseTypedLiteralTable.get(termType)}
      )
    }
    else {
  ...
  async _transformUntypedValue({entries, contextStack, value}) {
    const decoder = UntypedLiteralDecoder.createDecoder({value, transformer: this});
    entries.push(decoder.decode({value}));
  }
