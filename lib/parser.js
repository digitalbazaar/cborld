...
// hex = 0xcb1d; modern implementations only need to implement this tag
const TAG_1_0 = 51997;
// hex = 0x0600; rarely used tag
const TAG_LEGACY_RANGE_START = 1536;
// hex = 0x06ff; rarely used tag
const TAG_LEGACY_RANGE_END = 1791;
// hex = 0x0500; some legacy production software uses this tag
const TAG_LEGACY_UNCOMPRESSED = 1280;
// hex = 0x0501; some legacy production software uses this tag
...
class Parser extends Tokenizer {
  ...
  _varintToRegistryEntryId({nextToken}) {
    ...
    if(varintBytes.length >= 24) {
      throw new CborldError(
        'ERR_NOT_CBORLD', 'CBOR-LD encoded registry entry ID too large.');
    }
