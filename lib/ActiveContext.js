...
export class ActiveContext {
  ...
  async applyPropertyScopedContext({term}) {
    const activeCtx = await _updateActiveContext({
      ...
      // FIXME: determine if termMap can be used instead, removing extra
      // scoped context map processing
      contexts: this._activeCtx.scopedContextMap.get(term),
      contextLoader: this.contextLoader,
      propertyScope: true
    });
    return new ActiveContext({activeCtx, previous: this});
    ...
    for(const type of objectTypes) {
      ...
      if(contexts) {
        activeCtx = await _updateActiveContext({
          activeCtx, contexts, contextLoader: this.contextLoader,
          typeScope: true
        });
  ...
  getTypeTerms() {
    ...
    // FIXME: can type aliases just be computed on the fly from the `termMap`?
...
function _createNewActiveContext() {
  return {
    typeAliases: new Set(),
    // FIXME: determine if this is reliable after reverting, perhaps remove
...
function _revertActiveContext({activeCtx}) {
  ...
  const newTypeAliases = new Set();
  const newScopedContextMap = new Map();

  // keep every propagating term
  const {typeAliases, termMap, scopedContextMap} = activeCtx._activeCtx;
  ...
  for(const [term, def] of termMap) {
    ...
    if(typeAliases.has(term)) {
      newTypeAliases.add(term);
    }
    const scopedContext = scopedContextMap.get(term);
    if(scopedContext !== undefined) {
      newScopedContextMap.set(term, scopedContext);
    }
  }
  ...
  for(const term of nonPropagating) {
    ...
    const {typeAliases, scopedContextMap} = currentCtx._activeCtx;
    if(def !== undefined) {
      newTermMap.set(term, def);
      if(typeAliases.has(term)) {
        newTypeAliases.add(term);
  ...
  newActiveCtx.typeAliases = newTypeAliases;
...
async function _updateActiveContext({
  ...
  for(let context of contexts) {
    ...
    const {typeAliases, termMap} = newActiveCtx;
    ...
    const {scopedContextMap} = newActiveCtx;
