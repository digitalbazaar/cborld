...
export class Transformer {
  ...
  constructor({
    ...
    this.urlSchemeTable = urlSchemeTable;
  ...
  async _addContext({context, contextUrl}) {
    ...
    for(const key of keys) {
      ...
      if(this.urlSchemeTable.has(key)) {
        // skip STRING_TABLE
        continue;
      }
  ...
  _getTermForId({id}) {
    ...
    let term;
    // check keywords table
    if(this.reverseKeywordsTable.has(id)){
      term = this.reverseKeywordsTable.get(id);
    }
    else{
      term = this.reverseTableFromContexts.get(plural ? id - 1 : id);
    }
