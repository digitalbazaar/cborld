...
export class Transformer {
   ...
   */
  async _transform({activeCtx = this.initialActiveCtx, obj, transformMap}) {
    ...
    console.log('start', activeCtx._activeCtx);
    activeCtx = await activeCtx.applyEmbeddedContexts({obj});

    // get unique `@type` (and alias) values for the object
    const objectTypes = strategy._getObjectTypes({
      activeCtx, obj, transformMap
    });
    ...
    console.log('typeActiveCtx', typeActiveCtx._activeCtx);

    // walk term entries to transform
    const termEntries = strategy._getEntries({
      activeCtx: typeActiveCtx, obj, transformMap, transformer: this
    });
    for(const [termInfo, value] of termEntries) {
    //for(const [term, value] of termEntries) {
      const {term} = termInfo;
      console.log('\nprocessing term:', term);
      console.log('def', termInfo.def);
      ...
      console.log('propertyActiveCtx', propertyActiveCtx._activeCtx);

      // FIXME: remove
      // const def2 = propertyActiveCtx.getTermDefinition({term});
      // console.log('NEW def (should not change)', term, def2);
      ...
      for(const value of values) {
        ...
        if(Array.isArray(value)) {
          await strategy._transformArray({
            activeCtx: propertyActiveCtx, entries, value
          });
          continue;
        }

        // transform object
        console.log('recursing into object', value);
