...
import {STANDARDS_TABLE} from './keywords.js';
...
export class Transformer {
   ...
   * integers.
   */
  constructor({compressionMap, documentLoader} = {}) {
  ...
  async _addContext({context, contextUrl}) {
    const {contextMap, stringToId, idToString} = this;
    ...
    for(const key of keys) {
      ...
      if(STANDARDS_TABLE.has(key)) {
        // skip STANDARDS_TABLE
        continue;
      }
      // ensure the term has been assigned an ID
      if(!stringToId.has(key)) {
  ...
  _getIdForTerm({term, plural}) {
    const id = this.stringToId.get(term);
    if(id === undefined) {
      throw new CborldError(
        'ERR_UNDEFINED_TERM',
  ...
  _getTermForId({id}) {
    const plural = (id & 1) === 1;
    const term = this.idToString.get(plural ? id - 1 : id);
    return {term, plural};
  }
}
