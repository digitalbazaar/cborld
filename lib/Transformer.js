...
export class Transformer {
  ...
  constructor({
    ...
    this.typedLiteralTable = typedLiteralTable ?? new Map();

    // FIXME: clean up
    this.typedLiteralTable.set('url', stringTable);
    this.typedLiteralTable.set('none', stringTable);
  }
  ...
  async _transform({obj, transformMap, contextStack = []}) {
    ...
    for(const [termInfo, value] of termEntries) {
      ...
      //   console.log('_transformObjectId');
      ...
      //   console.log('_transformObjectType');
      ...
      for(const value of values) {
        ...
        // FIXME: try and unify better here with encoding / decoding; encoder
        // never needs an object nor an array, but decoder does
  ...
  async _addContext({context, contextUrl}) {
    ...
    for(const key of keys) {
      ...
      // FIXME: determine if `@id` or `@type` aliases have to be special cased
      // or not
