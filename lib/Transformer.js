...
export class Transformer {
    ...
    for(const [termInfo, value] of termEntries) {
      ...
      for(const value of values) {
        await this._transformValue({
          activeCtx: propertyActiveCtx,
          outputs: entries, termType, value, termInfo
        });
      }
  ...
  async _transformArray({activeCtx, entries, termType, values, termInfo}) {
    // recurse into array
    const outputs = [];
    for(const value of values) {
      await this._transformValue({
        activeCtx, outputs, termType, value, termInfo
      });
    }
  ...
  async _transformValue({activeCtx, outputs, termType, value, termInfo}) {
    ...
    if(this.strategy._transformValue({
      entries: outputs, termType, value, termInfo
    })) {
      return;
    }
    if(Array.isArray(value)) {
      ...
      return this._transformArray({
        activeCtx, entries: outputs, termType, values: value, termInfo
      });
    }
