...
export class Converter {
  ...
  async _transform({
    ...
    // walk each term => value entries to convert them
  ...
  async _transformValue({activeCtx, outputs, termType, value, termInfo}) {
    ...
    if(value === null) {
      // FIXME: if `_transformValue` just returns the transformed value,
      // then `outputs.push()` can be called once or even run in parallel,
      // provided that the results are kept in order
      outputs.push(null);
      return;
    }
    // transform value via strategy if possible
    let output = this.strategy._transformValue({termType, value, termInfo});
    if(output !== undefined) {
      outputs.push(output);
      return;
    }
    ...
    output = this.strategy._createNewOutput();
    outputs.push(output);
