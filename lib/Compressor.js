...
export class Compressor {
  ...
  async compress({jsonldDocument} = {}) {
    ...
    for(const input of docs) {
      // FIXME: only diff w/decompress() is creation of a new output
  ...
  async _transformArray({activeCtx, entries, value}) {
    ...
    for(const obj of value) {
      ...
      await this.transformer._transform({
        activeCtx, input: obj, output: childMap
      });
    }
  ...
  _assignEntries({entries, termInfo, output: transformMap}) {
    transformMap.set(termInfo.termId, entries);
  }
}
