...
export class Compressor extends Transformer {
  ...
  async _createTransformMaps({jsonldDocument}) {
    ...
    this.nextTermId = this._getFirstCustomId({compressionMap: this.compressionMap});
  ...
  _getEntries({obj, termMap}) {
    ...
    for(const key of keys) {
      ...
      if(def === undefined && !(key.startsWith('@') && STANDARDS_TABLE.has(key))) {
  ...
  _getFirstCustomId({compressionMap}) {
    //FIXME: remove and replace with check when iterating id if number already used
   return 100;
    /*
    let maxValue = 0;
