...
export class Compressor {
  ...
  _getEntries({activeCtx, obj}) {
    const {transformer: {contextLoader}} = this;
    ...
    for(const key of keys) {
      ...
      // get term definition
      // FIXME: do not access term map directly
      const def = activeCtx._activeCtx.termMap.get(key) ?? {};

      const value = obj[key];
      const plural = Array.isArray(value);
      // FIXME: note: this is the only place this is called; determine if
      // other uses of `contextLoader.termToId` should use it/be changed
      const termId = contextLoader.getIdForTerm({term: key, plural});
      entries.push([{term: key, termId, plural, def}, value]);
    }
    return entries;
  }

  _getObjectTypes({activeCtx, obj}) {
  ...
  async _transformArray({activeCtx, entries, value}) {
    // recurse into array
    const children = [];
    for(const obj of value) {
      if(Array.isArray(obj)) {
        ...
        await this._transformArray({
          activeCtx, entries: children, value: obj
        });
        continue;
      }
      // handle non-object
      ...
      await this.transformer._transform({
        activeCtx, obj, transformMap: childMap
      });
    }
    entries.push(children);
  }

  async _transformObject({activeCtx, entries, value}) {
    // recurse into object
    const transformMap = new Map();
    entries.push(transformMap);
    await this.transformer._transform({
      obj: value, transformMap, activeCtx
    });
  }
