...
import {TypeTableEncoder} from './codecs/TypeTableEncoder.js';
...
export class Compressor extends Transformer {
  ...
  constructor({
    ...
    // FIXME: if these tables are never modified, do not make copies here
  ...
  _getEntries({obj, termMap}) {
    ...
    for(const key of keys) {
      ...
      if(def === undefined) {
        /* DO NOT DELETE - might want to readd for debug purposes
  ...
  _transformObjectId({transformMap, termInfo, value}) {
    ...
    /*const encoder = TypeTableEncoder.createEncoder(
      {value, transformer: this, termInfo, termType: '@id'});*/
  ...
  _transformObjectType({transformMap, termInfo, value}) {
    ...
    for(const value of values) {
      ...
      /*const encoder = VocabTermEncoder.createEncoder(
        {value, transformer: this, termInfo});*/
      const encoder = TypeTableEncoder.createEncoder(
        {value, transformer: this, termInfo, termType: '@type'});
      entries.push(encoder || value);
    }
  ...
  async _transformUntypedValue({entries, value}) {
    console.log('called _transformUntypedValue', value);
    process.exit(1);
