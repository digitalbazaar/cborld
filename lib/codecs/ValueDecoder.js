...
export class ValueDecoder extends CborldDecoder {
  ...
  static createDecoder({value, transformer, termType, termInfo} = {}) {
    ...
    const subTable = transformer.reverseTypeTable.get(tableType);
    ...
    const decoder = PROCESSING_MODE_TYPE_DECODERS.get(tableType)?.createDecoder(
      {value, transformer, termInfo});
    if(decoder) {
      return decoder;
    }

    // if value is not an array, pass it through directly
    if(!Array.isArray(value)) {
      return new ValueDecoder({decoded: value});
    }
  }
}
