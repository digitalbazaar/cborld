...
export class HttpUrlEncoder extends CborldEncoder {
  ...
  static createEncoder({value, transformer} = {}) {
    ...
    if(value.startsWith('https://')) {
      return new HttpUrlEncoder({value, compressionMap: transformer.urlSchemeTable, secure: true});
    }
    if(value.startsWith('http://')) {
      return new HttpUrlEncoder({value, compressionMap: transformer.urlSchemeTable, secure: false});
    }
  }
}
