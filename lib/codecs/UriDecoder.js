...
export class UriDecoder extends CborldDecoder {
  static createDecoder({value, transformer} = {}) {
    if(!(Array.isArray(value) || value.length > 1)) {
      if(transformer.reverseStringTable.has(value) || transformer.reverseTableFromContexts.has(value)){
        return VocabTermDecoder.createDecoder({value, transformer});
      }
      return false;
    }

    const DecoderClass = SCHEME_ID_TO_DECODER.get(transformer.reverseUrlTable.get(value[0]));
