...
export class MultibaseDecoder extends CborldDecoder {
  ...
  decode({value} = {}) {
    ...
    if(this.reverseCompressionMap.has(value)) {
      return this.reverseCompressionMap.get(value);
    }
    if(value[0] === 0x7a) {
  ...
  static createDecoder({value, transformer} = {}) {
    ...
    if(value[0] === 0x7a || value[0] === 0x75 || value[0] === 0x4d) {
      return new MultibaseDecoder({reverseCompressionMap: reverseStringTable});
    }
  }
}
