...
export class UrlCodec {
  ...
  decodeCBOR() {
    ...
    // FIXME: This is fragile, encode compressed URLs as a CBOR map
    if(Array.isArray(this.value)) {
    ...
    //   this.value.map(element => _decodeUrl(element, this.termCodecMap)) :
    //   _decodeUrl(this.value, this.termCodecMap);
...
export class Base58DidUrlCodec {
  ...
  decode(value) {
    ...
    console.log(".decode()", [encodedHeader, encodedAuthority, encodedFragment]);
    const header = urlHeaders.get(encodedHeader);
    const authority = b58.encode(Uint8Array.from(encodedAuthority));
    const fragment = (encodedFragment) ? b58.encode(Uint8Array.from(encodedFragment)) : undefined;
...
function _decodeUrl(encodedUrl, termCodecMap) {
  console.log("DECODE URL", encodedUrl);
