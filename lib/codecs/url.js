...
export class Base58DidUrlCodec {
  ...
  decode(value) {
    ...
    const authority = b58.encode(encodedAuthority);
    const fragment = b58.encode(encodedFragment);

    return `${header}:${authority}#${fragment}`;
  }
}
...
function _decodeUrl(encodedUrl, termCodecMap) {
  ...
  const decoders = urlEncoders.filter(element => encodedUrl.match(element[0]));
