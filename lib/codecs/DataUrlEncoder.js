...
export class DataUrlEncoder extends CborldEncoder {
  constructor({value, base64, compressionMap} = {}) {
    super();
    this.value = value;
    this.base64 = base64;
  ...
  encode() {
    const {value, base64, compressionMap} = this;

    const entries = [
      new Token(Type.uint, compressionMap.get('data:')),
    ];
  ...
  static createEncoder({value, transformer} = {}) {
    ...
    if(parsed) {
      ...
      if(data === Base64.fromUint8Array(Base64.toUint8Array(data))) {
        return new DataUrlEncoder({value, base64: true, compressionMap: transformer.urlSchemeTable});
      }
    }
    return new DataUrlEncoder({value, base64: false, compressionMap: transformer.urlSchemeTable});
  }
}
