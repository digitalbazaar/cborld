...
export class VocabTermDecoder extends CborldDecoder {
  ...
  static createDecoder({value, transformer} = {}) {
    ...
    let term;
    if(transformer.reverseKeywordsTable.has(value)){
      term = transformer.reverseKeywordsTable.get(value);
    }
    else if(transformer.reverseStringTable.has(value)){
      term = transformer.reverseStringTable.get(value);
    }
    else if(transformer.reverseTableFromContexts.has(value)) {
      term = transformer.reverseTableFromContexts.get(value)
    }
    if(term !== undefined) {
