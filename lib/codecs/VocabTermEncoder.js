...
export class VocabTermEncoder extends CborldEncoder {
  ...
  static createEncoder({value, transformer} = {}) {
    let termId;
    if(transformer.keywordsTable.has(value)){
      termId = transformer.keywordsTable.get(value);
    }
    else if(transformer.stringTable.has(value)){
      termId = transformer.stringTable.get(value);
    }
    else if(transformer.tableFromContexts.has(value)) {
