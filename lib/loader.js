...
function _get(url, file, resolve, reject) {
  const options = {
    headers: {
  ...
  https.get(url, options, res => {
    if(res.statusCode === 301 || res.statusCode === 302) {
      return _get(res.headers.location, file, resolve, reject);
    } else {
      res.pipe(file);
      res.on('end', resolve);
      res.on('error', reject);
...
export async function documentLoader(docUrl) {
  ...
  try {
    ...
    await new Promise((resolve, reject) => _get(docUrl, file, resolve, reject));
  }
